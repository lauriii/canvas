...
export const edit = (cy) => {
  ...
  cy.findByLabelText('XB Text Area with Summary').as('textarea-summary');
  // And the wrapper.
...
export const assertData = (response) => {
  ...
  expect(response.attributes.field_xbt_textarea_summary.value).to.eq(newValue);
  expect(response.attributes.field_xbt_textarea_summary.summary).to.eq(
    newSummary,
  );
};
