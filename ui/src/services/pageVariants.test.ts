...
describe('resolvePageVariantSelection', () => {
  it('uses an unsaved explicit selection instead of the layout response', () => {
    expect(
      resolvePageVariantSelection('saved', 'pending', 'site_default'),
    ).toBe('pending');
  });

  it('uses the site default for an empty selection', () => {
    expect(resolvePageVariantSelection('saved', '_none', 'site_default')).toBe(
      'site_default',
    );
    expect(resolvePageVariantSelection('saved', '', 'site_default')).toBe(
      'site_default',
    );
    expect(resolvePageVariantSelection('saved', null, 'site_default')).toBe(
      'site_default',
    );
  });

  it('hides the layer when the site has no default variant', () => {
    expect(resolvePageVariantSelection('saved', '_none', null)).toBeNull();
  });

  it('retains the layout response until page data and settings load', () => {
    expect(resolvePageVariantSelection('saved', undefined, undefined)).toBe(
      'saved',
    );
    expect(resolvePageVariantSelection('saved', '_none', undefined)).toBe(
      'saved',
    );
  });
});
