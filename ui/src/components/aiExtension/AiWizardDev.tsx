...
const progressToHtml = (progress: string, isFinished: boolean): string => {
  ...
  return `${narration}<div class="aiProgressStatus"><span class="${icon}"></span>${Drupal.t('Thinking')}</div>`;
};
...
const DEEP_CHAT_TEXT_INPUT = {
  placeholder: { text: Drupal.t('Build me a ...') },
  styles: {
    text: {
      padding: '16px',
    },
        ...
        <Flex direction="column" align="center">
          ...
          <Flex direction="row" align="center" gap="0">
            <Box className={styles.aiWizardTitleContainer}>
              ...
              <Text className={styles.aiWizardBeta}>{Drupal.t('Beta')}</Text>
            </Box>
          </Flex>
          <Text ref={welcomeTextRef} className={styles.aiWizardSubtitle}>
            {Drupal.t('Hello, how can I help you today?')}
          </Text>
        </Flex>
        <MemoDeepChat
          ref={chatElementRef}
