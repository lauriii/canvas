...
const FormElement = ({
    ...
    titleDisplay = '',
    children = '',
    renderChildren = ''}: FormElementProps) => {
  const classes = clsx(
    'js-form-item',
    'form-item',
    `js-form-type-${type}`,
  ...
  return <div {...a2p(attributes, {'class': classes})}>
    {['before', 'invisible'].includes(labelDisplay) && label}
    {prefix && prefix.length > 0 && <span className="field-prefix">{prefix}</span>}
    {(descriptionDisplay === 'before' && description.content) && description.content &&
      <div {...a2p(description.attributes || {}, {'class': descriptionClasses})}>
        {description.content}
      </div>
    }
    {renderChildren}

    {suffix && suffix.length > 0 && <span className="field-suffix">{suffix}</span>}
    {['after'].includes(labelDisplay) && label}

    {errors && <div className="form-item--error-message form-item-errors">
      {errors}
    </div>}
