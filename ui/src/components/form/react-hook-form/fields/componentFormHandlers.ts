...
import { dispatchComponentPropEdit } from '@/features/layout/preview/partialRender';
import { setPreviewBackgroundUpdate } from '@/features/pagePreview/previewSlice';
import {
  getPreviewPerformanceFlags,
  PREVIEW_CADENCE,
} from '@/utils/previewCadence';
...
export const POLLED_BACKGROUND_TIMEOUT =
  PREVIEW_CADENCE.ISLAND_BACKGROUND_UPDATE_DELAY;
...
export const createComponentFormStateHandler = (
  ...
  return (
    ...
    if (isEvaluatedComponentModel(selectedModel) && deps.component) {
      previewPerfMark('prop-edit-debounce-fire', {
        uuid: deps.selectedComponent,
      });
      const payload = {
      ...
      const updateBackend = () => {
        deps.patchComponent(newInputAndUiData, payload);
      };
      if (backgroundPreviewUpdate) {
        if (deps.polledBackgroundUpdate.current !== null) {
          clearTimeout(deps.polledBackgroundUpdate.current);
        }
        // The island already painted optimistically; only the authoritative
        // PATCH is debounced here. If we're not doing a background preview
        // update, we should schedule this immediately - debouncing in
        // withRHF will handle preventing this firing too many times in
        // succession.
      ...
      // Bounded server-rendered edit: route through the partial render
      // endpoint (blast radius: this component's subtree) with debounced
      // persistence; anything the partial path cannot serve falls back to
      // the legacy PATCH flow.
      const flags = getPreviewPerformanceFlags();
      deps.dispatch(
        dispatchComponentPropEdit({
          uuid: deps.selectedComponent,
          model: payload,
          isEntityContext: deps.editorFrameContext === 'entity',
          partialRenderEnabled: flags.partialRender && flags.decoupledAutoSave,
          fallbackPatch: updateBackend,
        }) as any,
      );
      return;
    }
