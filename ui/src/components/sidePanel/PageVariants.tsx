...
const PageVariants = () => {
  ...
  const handleDelete = async (variant: PageVariant) => {
    const result = await deleteVariant(variant.id);
    // Deleting the variant that is open in the editor leaves the editor on a
    // dead route; move to the next best page, as template deletion does.
    if (
      result &&
      !result.error &&
