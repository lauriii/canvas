...
const DummyPropsEditForm = () => {
  ...
  const components:ComponentsList = componentsResults.data ? componentsResults.data : {};
  ...
  const selectedComponentFieldData:PropDataCollection = components[selectedComponentType]['field_data'] || {};

  // The prepared model combines prop values from the model and prop metadata
  // from the SDC definition.
  for (const [thePropName, propData] of Object.entries(selectedComponentFieldData)) {
    ...
    if (propData.sourceType) {
      preparedModel[selectedComponent as keyof PreparedModel][propName].sourceType = propData.sourceType;
    }
  ...
  const tree = layout.children.filter((node) => node.uuid === selectedComponent);
  ...
  const responseAsDocument = new DOMParser().parseFromString(response.data as string, 'text/html');
  // Get the form we want from the HTML response.
  const xbDemoFieldElement: HTMLTemplateElement|null = responseAsDocument.querySelector('template[hyperscriptify]');
  const jsxFormContent = xbDemoFieldElement?.content ? hyperscriptify(xbDemoFieldElement?.content as DocumentFragment, React.createElement, React.Fragment, twigToJSXComponentMap, {propsify}) : null;
  ...
  return (
    <>
      <h3 style={{color: 'white', fontFamily: 'monospace'}}>Here is DummyPropsEditForm 👇</h3>
      <b style={{color: 'white', fontFamily: 'monospace'}}>We know this is not an ideal form. This is a work in progress.</b>
      {jsxFormContent ? jsxFormContent : <h1 style={{color: 'white'}}>Loading...</h1>}
    </>
  );
};
