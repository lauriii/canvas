...
describe('Code editor utilities', () => {
  ...
  describe('minItems: 1 for required array props', () => {
    it('serializes minItems: 1 when set on an array prop', () => {
      const prop = {
        id: 'test-id',
        name: 'Tags',
        type: 'array',
      ...
      const result = serializeProps([prop]);
    ...
    it('does not serialize minItems when it is not set on an array prop', () => {
      const prop = {
        id: 'test-id',
        name: 'Tags',
        type: 'array',
      ...
      const result = serializeProps([prop]);
      expect(result.tags.minItems).toBeUndefined();
    });

    it('deserializes minItems: 1 from a serialized array prop', () => {
      const [result] = deserializeProps([
        {
          title: 'Tags',
          type: 'array',
    ...
    it('deserializes without minItems when not present in serialized array prop', () => {
      const [result] = deserializeProps([
        {
          title: 'Tags',
          type: 'array',
    ...
    it('round-trips: serializing a deserialized prop with minItems: 1 preserves minItems', () => {
      const serialized = {
        title: 'Tags',
        type: 'array',
        items: { type: 'string' },
      ...
      expect(deserialized.minItems).toBe(1);
      const serializedAgain = serializeProps([deserialized]);
      expect(serializedAgain.tags.minItems).toBe(1);
    });
  });
