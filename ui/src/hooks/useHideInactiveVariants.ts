...
/**
 * Hides the inactive personalization cases in the preview. The server renders
 * every case of a switch, so the editor hides all but the previewed variant's
 * case client-side. The boundary of the active case is not marked here: the
 * personalized section overlay draws it above the iframe, where the preview's
 * own styles cannot clip it. The effect re-runs whenever the preview DOM map
 * refreshes (the iframe is replaced on every preview round trip), which
 * reapplies the hiding to the new elements.
 */
export function useHideInactiveVariants() {
  ...
  useEffect(() => {
    ...
    // Restore every hidden element on cleanup so a stale hide never sticks to
    // an element after the previewed variant or the DOM map changes.
    return () => {
