...
3. [Data model](https://www.drupal.org/project/issues/experience_builder?component=Data+model)
...
- [`XB Data Model` doc](data-model.md)
...
Integrating `Redux` with the `component prop`s form (and likely other forms in the future) allows us to integrate Drupal Form API-generated forms into the larger XB application. The reasons for this include:
...
If you have not yet read [`XB Data Model` doc](data-model.md), and specifically section 3.1.2 ("Finding fitting
`field type`: `conjured field`s and `field instance`s") because the data model parts are less relevant/interesting, that
is fine. Here's a summary that explains why this document/infrastructure is needed:]
1. XB is about composing components (into a `component tree`) to craft an experience
2. a `component` has `component prop`s, which must be populated to render the `component`
3. XB aims to use Drupal's existing `field type` functionality to store values for those `component prop`s
4. for XB to choose a `field type` that is appropriate for a particular `component prop`, XB analyzes its `prop shape`
...
intermediary concepts (`field widget`, `field type`, `field prop`, etc.) into a `component prop`, to achieve real-time
 previews.**
...
- Client-side validation based on the [JSON Schema definition](https://json-schema.org/understanding-json-schema) of each `component prop`.
...
### 3.3 The spectrum of `HTML form control element` → `component prop` flows

A `component prop` has a `prop shape`, resulting in >=1 `field prop`s being used, which needs 1 `field widget` that has
...
1. **Number of `field prop`s:** non-list `prop shape`s: the shape of the value needed by a `component prop` ranges between:
...
3. **Computed `field prop`s:** a `field item` that has >=1 _computed_ `field prop`s, _if_ needed by the `component prop`.
