...
<LinkButton
  href="https://git.drupalcode.org/project/canvas/-/blob/0.x/ui/lib/astro-hydration/src/lib/utils.ts"
  variant="minimal"
  icon="external"
>
  Source Code
...
<LinkButton
  href={`${import.meta.env.BASE_URL}/code-components/data-fetching`.replace(
    /^\/\//,
    '/',
  )}
  variant="secondary"
