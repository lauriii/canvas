...
4. `+T` Supporting "exposed slots defined in content templates" functionality — realized in [ADR 0019](0019-exposed-slots-store-per-entity-content-in-per-slot-fields.md): each exposed slot is its own `component_tree` field on the bundle, and an entity stores its per-entity content for the slot as an ordinary tree in that field, merged into the template's target slot at render time
  - Slot field machine names are shared bundle-wide, but cannot collide across content templates: in Canvas 1.x only the full view mode's template may expose slots (out-of-scope item #1), so at most one template per bundle exposes slots, and the `canvas_slot_`-prefixed machine name is chosen by the user when exposing a slot — creating a new slot field under a name that already exists on the bundle is rejected (deliberately re-using an existing *unreferenced* `component_tree` field is a separate, explicit choice). If additional view modes ever gain exposed slots, the user-chosen machine name remains the disambiguator.
...
### 2026-07-16
One clarification added after acceptance; no decisions were changed. Consequence #4 received a note on slot-field machine-name collisions, prompted by review of [!1359](https://git.drupalcode.org/project/canvas/-/merge_requests/1359): collisions across content templates cannot arise in Canvas 1.x because only the full view mode's template may expose slots (out-of-scope item #1), and the user-chosen machine name remains the disambiguator if additional view modes ever gain exposed slots.
