...
4. `+T` Supporting "exposed slots defined in content templates" functionality — realized in [ADR 0021](0021-exposed-slots-store-per-entity-content-in-per-slot-fields.md): each exposed slot is its own `component_tree` field on the bundle, and an entity stores its per-entity content for the slot as an ordinary tree in that field, merged into the template's target slot at render time
