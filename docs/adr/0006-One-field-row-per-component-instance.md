...
4. `+T` Supporting "exposed slots defined in content templates" functionality is as simple as allowing `slot` to be an exposed slot defined by the `ContentTemplate` config entity — realized in [ADR 0019](0019-exposed-slots-alias-keyed-per-entity-subtrees.md): a per-entity row stores the exposed slot's stable alias in `slot` with an empty `parent_uuid` ("bonsai" root)
