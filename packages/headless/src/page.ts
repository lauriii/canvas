...
import type { CanvasContext as CanvasContextData } from 'drupal-canvas/drupal-utils';
 ...
 * `usePageContext()` and `useSiteContext()` hooks of the `drupal-canvas`
 * package. Generated by Drupal for the routed request; application authors
 * pass it to the rendering integration (`context={page.context}`), which
 * establishes the provider. The shapes match `getPageData()` and
 * `getSiteData()`. Theme assets come from Drupal's configured default theme;
 * frontends choose whether to use them and must still allow empty asset URLs.
 */
export type CanvasContext = CanvasContextData;
