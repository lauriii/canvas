...
final class PageVariantAccessControlHandler extends CanvasConfigEntityAccessControlHandler {
   ...
   * {@inheritdoc}
   *
   * Content templates depending on a variant must not block its deletion:
   * ContentTemplate::onDependencyRemoval() drops the selection (falling back
   * to the site default) instead of cascade-deleting the template, so that
   * graceful path stays reachable through the HTTP API too.
   *
  ...
  protected static function getEntityTypesAllowedToDependOnDeletedEntities(): array {
    return [...parent::getEntityTypesAllowedToDependOnDeletedEntities(), ContentTemplate::ENTITY_TYPE_ID];
  }
