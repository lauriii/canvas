...
class ComponentTreeHydrated extends TypedData implements CacheableDependencyInterface, RenderableInterface {
  ...
  public function toRenderable(bool $isPreview = FALSE): array {
    ...
    return self::renderify($hydrated, $isPreview);
  }
   ...
   */
  private static function renderify(array $hydrated, bool $isPreview = FALSE) {
    $build = [];
    foreach ($hydrated as $component_subtree_uuid => $component_instances) {
      foreach ($component_instances as $component_instance_uuid => $component_instance) {
        ...
        assert($component instanceof Component);
        $source = $component->getComponentSource();
        $build[$component_subtree_uuid][$component_instance_uuid] = $source->renderComponent($component_instance, $component_instance_uuid, $isPreview);
