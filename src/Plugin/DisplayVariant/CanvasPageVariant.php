...
final class CanvasPageVariant extends VariantBase implements PageVariantInterface, ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    // an empty title when the rendered main content provides no `#title` and
    // the route has no `_title`/`_title_callback` (for example the node
    // revision route). Core's own page display variants tolerate this, so this
    // one must too: any TitleBlockPluginInterface block simply receives the
    // empty title.
