describe(
  ...
  () => {
    ...
    it('Can zoom the canvas with the Zoom Controls', () => {
      ...
      cy.log('Selecting a "parent" component should show its label, but not the label(s) of its children')
      ...
      cy.log('After selecting a "child" component it should show its label and the "add component" button.')
      cy.get('@desktopPreviewOverlay').within(() => {
        ...
        cy.findAllByText('Image').should('have.length', 2).and('not.be.visible');
        cy.findAllByLabelText('Hero').eq(0).within(() => {
          cy.findByText('Add component');
        })
      });

      cy.log('Now hover a different component. The selected name should show, the hovered name should ALSO show.')
      cy.get('@desktopPreviewOverlay').within(() => {
        cy.findAllByLabelText('Image').eq(1).realHover({scrollBehavior: 'center'});
        cy.findAllByText('Hero').filter(':visible').should('have.length', 1);
